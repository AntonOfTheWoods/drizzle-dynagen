...
import { P as De, U as _n, F as bn, g as Nn, V as sn, h as En, j as Sn, k as $n, u as An, l as Ue, S as se, f as Le, m as Tn, n as wn } from "../view-D8huABrM.js";
import { e as ge, T as Ie, i as x, g as Ke } from "../casing-BBE-_e8-.js";
var Ge;
Ge = ge;
class an {
  constructor(i, r) {
...
class on {
  ...
  getName() {
    return this.name ?? `${this.table[De.Symbol.Name]}_${this.columns.map((i) => i.name).join("_")}_pk`;
  }
}
...
J(mn, tn, "PgIndex");
function ke(n) {
  const i = Object.values(n[Ie.Symbol.Columns]), r = [], u = [], N = [], b = Object.values(n[De.Symbol.InlineForeignKeys]), ae = [], le = n[Ie.Symbol.Name], O = n[Ie.Symbol.Schema], E = [], I = n[De.Symbol.EnableRLS], G = n[De.Symbol.ExtraConfigBuilder];
  if (G !== void 0) {
    const ee = G(n[Ie.Symbol.ExtraConfigColumns]), he = Array.isArray(ee) ? ee.flat(1) : Object.values(ee);
    for (const W of he)
      x(W, ln) ? r.push(W.build(n)) : x(W, cn) ? u.push(W.build(n)) : x(W, _n) ? ae.push(W.build(n)) : x(W, an) ? N.push(W.build(n)) : x(W, bn) ? b.push(W.build(n)) : x(W, Nn) && E.push(W);
  }
  return {
    columns: i,
    indexes: r,
...
const Fn = (n, i, r, u, N, b, ae, le, O, E) => {
  var k, Ne, Ae, Ee, Te, Y, we, ye, Q, Oe, Se, pe, xe, ve, qe;
  const I = new $n({ casing: O }), G = {}, ee = {}, he = {}, W = {}, de = {}, me = {};
  for (const e of n) {
    const d = {}, {
      name: m,
      columns: a,
    ...
    a.forEach((t) => {
      ...
      const h = fe(t, O), f = t.notNull, S = t.primary, s = t.getSQLType().toLowerCase(), c = x(t, Ue) ? t.enum.schema || "public" : void 0, y = t.generated, p = t.generatedIdentity, _ = M((C = p == null ? void 0 : p.sequenceOptions) == null ? void 0 : C.increment) ?? "1", $ = M((ne = p == null ? void 0 : p.sequenceOptions) == null ? void 0 : ne.minValue) ?? (parseFloat(_) < 0 ? Ye(t.columnType) : "1"), H = M((ue = p == null ? void 0 : p.sequenceOptions) == null ? void 0 : ue.maxValue) ?? (parseFloat(_) < 0 ? "-1" : Me(t.getSQLType())), T = M((ce = p == null ? void 0 : p.sequenceOptions) == null ? void 0 : ce.startWith) ?? (parseFloat(_) < 0 ? H : $), w = M((Ce = p == null ? void 0 : p.sequenceOptions) == null ? void 0 : Ce.cache) ?? "1", P = {
        ...
        notNull: f,
        generated: y ? {
          as: x(y.as, se) ? I.sqlToQuery(y.as).sql : typeof y.as == "function" ? I.sqlToQuery(y.as()).sql : y.as,
          type: "stored"
        } : void 0,
        identity: p ? {
          type: p.type,
          ...
          maxValue: H,
          cache: w,
          cycle: ((Re = p == null ? void 0 : p.sequenceOptions) == null ? void 0 : Re.cycle) ?? !1
        } : void 0
      };
      if (t.isUnique) {
        const We = o[t.uniqueName];
        if (typeof We < "u")
          throw console.error(
            `
${te.errorWarning(`We've found duplicated unique constraint names in ${m} table.
          The unique constraint ${t.uniqueName} on the ${h} column is conflicting with a unique constraint name already defined for ${We.columns.join(
              ","
            )} columns
`)}`
          ), new Error();
...
`
      ...
      const h = m, f = t.onDelete, S = t.onUpdate, s = t.reference(), c = Ke(s.foreignTable), y = ke(s.foreignTable).schema, p = s.columns.map((w) => w.name), _ = s.columns.map((w) => fe(w, O)), $ = s.foreignColumns.map((w) => w.name), H = s.foreignColumns.map((w) => fe(w, O));
      let T = t.getName();
      if (O !== void 0) {
      ...
      h.forEach((c) => {
        if (x(c, se) && typeof t.config.name > "u")
          throw console.log(
            `
${te.errorWarning(
              `Please specify an index name in ${Ke(t.config.table)} table that has "${I.sqlToQuery(c).sql}" expression. We can generate index names for indexes on columns only; for expressions in indexes, you need to specify the name yourself.`
            )}`
          ), new Error();
        c = c;
        const y = fe(c, O);
...
`
      ...
      const S = t.config.name ? t.config.name : qn(m, f), s = h.map((c) => {
        var y, p, _, $, H;
        return x(c, se) ? {
          expression: I.sqlToQuery(c, "indexes").sql,
          asc: !0,
          isExpression: !0,
          nulls: "last"
        } : (c = c, {
...
${te.errorWarning(
      ...
      j[S] = {
        ...
        where: t.config.where ? I.sqlToQuery(t.config.where).sql : void 0,
...
${te.errorWarning(
      ...
      v[t.name] = {
        ...
        to: h.sort(),
        using: x(t.using, se) ? I.sqlToQuery(t.using).sql : void 0,
        withCheck: x(t.withCheck, se) ? I.sqlToQuery(t.withCheck).sql : void 0
      };
    }), L.forEach((t) => {
      const h = t.name;
...
${te.errorWarning(
      ...
      ie[h] = {
        name: h,
        value: I.sqlToQuery(t.value).sql
      };
    });
    const F = `${A ?? "public"}.${m}`;
    G[F] = {
...
${te.errorWarning(
    ...
    const l = {
      ...
      to: a.sort(),
      using: x(e.using, se) ? I.sqlToQuery(e.using).sql : void 0,
      withCheck: x(e.withCheck, se) ? I.sqlToQuery(e.withCheck).sql : void 0
    };
  ...
  const _e = [...ae, ...le];
  for (const e of _e) {
    let d, m, a, l, q, L, A, V, R, X = !1;
    x(e, Tn) ? { name: d, schema: m, query: a, selectedFields: l, isExisting: q, with: L } = On(e) : ({
      name: d,
      schema: m,
      query: a,
      selectedFields: l,
...
${te.errorWarning(
    ...
    for (const g in l)
      if (x(l[g], wn)) {
        const o = l[g], v = o.notNull, K = o.primary, F = o.getSQLType().toLowerCase(), t = x(o, Ue) ? o.enum.schema || "public" : void 0, h = o.generated, f = o.generatedIdentity, S = M((Oe = f == null ? void 0 : f.sequenceOptions) == null ? void 0 : Oe.increment) ?? "1", s = M((Se = f == null ? void 0 : f.sequenceOptions) == null ? void 0 : Se.minValue) ?? (parseFloat(S) < 0 ? Ye(o.columnType) : "1"), c = M((pe = f == null ? void 0 : f.sequenceOptions) == null ? void 0 : pe.maxValue) ?? (parseFloat(S) < 0 ? "-1" : Me(o.getSQLType())), y = M((xe = f == null ? void 0 : f.sequenceOptions) == null ? void 0 : xe.startWith) ?? (parseFloat(S) < 0 ? c : s), p = M((ve = f == null ? void 0 : f.sequenceOptions) == null ? void 0 : ve.cache) ?? "1", _ = {
          name: o.name,
          type: o.getSQLType(),
          typeSchema: t,
          primaryKey: K,
          notNull: v,
          generated: h ? {
            as: x(h.as, se) ? I.sqlToQuery(h.as).sql : typeof h.as == "function" ? I.sqlToQuery(h.as()).sql : h.as,
            type: "stored"
          } : void 0,
          identity: f ? {
            type: f.type,
...
`
    ...
    ee[D] = {
      columns: j,
      definition: q ? void 0 : I.sqlToQuery(a).sql,
      name: d,
      schema: U,
      isExisting: q,
      with: L,
...
const Hn = async (n, i = () => !0, r, u, N, b) => {
  const ae = {}, le = {}, O = {}, E = { tables: {} }, I = r.map((e) => `n.nspname = '${e}'`).join(" or "), G = await n.query(
    `SELECT
...
WHERE
  ...
  let W = 0, de = 0, me = 0, _e = 0, be = 0, $e = 0;
  ...
  for (const e of Ae) {
    const d = e.schemaname, m = e.sequencename, a = B(e.start_value), l = B(e.min_value), q = B(e.max_value), L = B(e.increment_by), A = e.cycle, V = B(e.cache_size), R = `${d}.${m}`;
    k[R] = {
      name: m,
      schema: d,
      startWith: a,
  ...
  const we = await n.query("SELECT rolname, rolinherit, rolcreatedb, rolcreaterole FROM pg_roles;"), ye = {}, Q = Cn(u);
  ...
  for (const e of xe) {
    const { tablename: d, schemaname: m, to: a, withCheck: l, using: q, ...L } = e, A = pe[`${m}.${d}`], V = typeof a == "string" ? a.slice(1, -1).split(",") : a, R = l === null ? void 0 : l, X = q === null ? void 0 : q;
    A ? A[e.name] = { ...L, to: V } : pe[`${m}.${d}`] = {
      [e.name]: {
        ...L,
        to: V,
        withCheck: R,
  ...
  N && N(
    ...
    try {
      const U = {}, D = {}, j = {}, ie = {}, oe = {}, g = {}, o = await Be({
        schema: l,
        table: a,
        db: n
      }), v = await n.query(
      ...
      W += o.length, N && N("columns", W, "fetching");
      ...
      for (const s of F) {
        const c = s.column_name, y = s.foreign_table_name, p = s.foreign_column_name, _ = s.foreign_table_schema, $ = s.constraint_name, H = (q = s.update_rule) == null ? void 0 : q.toLowerCase(), T = (L = s.delete_rule) == null ? void 0 : L.toLowerCase();
      ...
      const t = v.filter((s) => s.constraint_type === "UNIQUE");
      for (const s of t) {
      ...
      for (const s of o) {
        const c = s.column_name, y = s.additional_dt, p = s.array_dimensions, _ = s.enum_name, $ = s.data_type, H = s.type_schema, T = s.column_default, w = s.is_generated === "ALWAYS", P = s.generation_expression, C = s.is_identity === "YES", ne = s.identity_generation === "ALWAYS" ? "always" : "byDefault", ue = s.identity_start, ce = s.identity_increment, Ce = s.identity_maximum, Re = s.identity_minimum, We = s.identity_cycle === "YES", Z = s.seq_name, un = v.filter(
          (re) => c === re.column_name && re.constraint_type === "PRIMARY KEY"
        ), Fe = v.filter((re) => re.constraint_type === "PRIMARY KEY");
        if (Fe.length > 1) {
        ...
        z = z.replace("character varying", "varchar").replace(" without time zone", "").replace("character", "char"), z = Qe(z, '"'), U[c] = {
          ...
          identity: C ? {
            ...
            maxValue: B(Ce),
            startWith: B(ue),
            cache: (A = k[Z]) != null && A.cache ? (V = k[Z]) == null ? void 0 : V.cache : (R = k[`${l}.${Z}`]) != null && R.cache ? (X = k[`${l}.${Z}`]) == null ? void 0 : X.cache : void 0,
            cycle: We,
            schema: l
          } : void 0
    ...
    END AS is_expression,
      ...
      for (const s of h) {
        const c = s.indexname, y = s.column_name, p = s.is_unique, _ = s.method, $ = s.with, H = s.where, T = s.opcname, w = s.is_expression === 1, P = s.descending, C = s.nulls_first, ne = {};
        $ !== null && $.forEach((ue) => {
          const ce = ue.split("=");
          ne[ce[0]] = ce[1];
        }), !S.includes(c) && (typeof D[c] < "u" ? D[c].columns.push({
          expression: y,
          asc: !P,
          nulls: C ? "first" : "last",
          opclass: T,
      ...
      de += Object.keys(D).length, N && N("indexes", de, "fetching"), ae[`${l}.${a}`] = {
        name: a,
        schema: l !== "public" ? l : "",
        columns: U,
        indexes: D,
    ...
    try {
      ...
      for (const g of X) {
        ...
        const ue = t === "serial";
        if (C.startsWith("numeric(") && (C = C.replace(",", ", ")), v === "ARRAY")
          for (let ce = 1; ce < Number(K); ce++)
            C += "[]";
        C = C.replace("character varying", "varchar").replace(" without time zone", "").replace("character", "char"), C = Qe(C, '"'), R[o] = {
          name: o,
          type: (
            // filter vectors, but in future we should filter any extension that was installed by user
            v === "USER-DEFINED" && !["vector", "geometry"].includes(F) ? F : C
          ),
          ...
          identity: s ? {
            ...
            maxValue: B(_),
            startWith: B(y),
            cache: (q = k[T]) != null && q.cache ? (L = k[T]) == null ? void 0 : L.cache : (A = k[`${l}.${T}`]) != null && A.cache ? (V = k[`${l}.${T}`]) == null ? void 0 : V.cache : void 0,
            cycle: H,
            schema: l
          } : void 0
        }, T && (delete k[`${l}.${T.startsWith('"') && T.endsWith('"') ? T.slice(1, -1) : T}`], delete k[T]), !ue && typeof ne < "u" && (R[o].default = ne);
      }
...
WHERE
    ...
    AND c.relname = '${a}';`), D = {};
      U.options && U.options.forEach((g) => {
        const o = g.split("="), v = o[0], K = o[1];
        K === "true" ? D[v] = !0 : K === "false" ? D[v] = !1 : isNaN(Number(K)) ? D[v] = K : D[v] = Number(K);
      });
      const j = U.definition.replace(/\s+/g, " ").replace(";", "").trim(), ie = Object.values(D).length ? Object.fromEntries(Object.entries(D).map(([g, o]) => [yn(g), o])) : void 0, oe = e.type === "materialized_view";
      le[`${l}.${a}`] = {
        name: a,
        schema: l,
        columns: R,
    ...
    d("");
  })).length, N && (N("columns", W, "done"), N("indexes", de, "done"), N("fks", me, "done"), N("checks", be, "done"), N("views", $e, "done"));
