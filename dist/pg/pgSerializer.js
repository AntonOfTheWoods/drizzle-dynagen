...
import { P as Ie, U as _n, F as bn, g as Nn, V as sn, h as En, j as Sn, k as $n, u as An, l as Ue, S as se, f as We, m as Tn, n as wn } from "../view-B6HOrUDb.js";
import { e as ge, T as Le, i as x, g as Ke } from "../casing-tDA94lCu.js";
var Ge;
Ge = ge;
class an {
  constructor(i, r) {
...
J(mn, tn, "PgIndex");
function ke(n) {
  const i = Object.values(n[Le.Symbol.Columns]), r = [], u = [], N = [], b = Object.values(n[Ie.Symbol.InlineForeignKeys]), ae = [], le = n[Le.Symbol.Name], O = n[Le.Symbol.Schema], E = [], L = n[Ie.Symbol.EnableRLS], G = n[Ie.Symbol.ExtraConfigBuilder];
  if (G !== void 0) {
    const ee = G(n[Le.Symbol.ExtraConfigColumns]), he = Array.isArray(ee) ? ee.flat(1) : Object.values(ee);
    for (const R of he)
      x(R, ln) ? r.push(R.build(n)) : x(R, cn) ? u.push(R.build(n)) : x(R, _n) ? ae.push(R.build(n)) : x(R, an) ? N.push(R.build(n)) : x(R, bn) ? b.push(R.build(n)) : x(R, Nn) && E.push(R);
  }
  return {
    columns: i,
    indexes: r,
      ...
      h.forEach((c) => {
        if (x(c, se) && typeof t.config.name > "u")
          throw console.log(
            `
${te.errorWarning(
              `Please specify an index name in ${Ke(t.config.table)} table that has "${L.sqlToQuery(c).sql}" expression. We can generate index names for indexes on columns only; for expressions in indexes, you need to specify the name yourself.`
            )}`
          ), new Error();
        c = c;
        const y = fe(c, O);
...
${te.errorWarning(
      ...
      var f, S;
      const h = [];
      if (t.to ? t.to && typeof t.to == "string" ? h.push(t.to) : t.to && x(t.to, We) ? h.push(t.to.name) : t.to && Array.isArray(t.to) && t.to.forEach((s) => {
        typeof s == "string" ? h.push(s) : x(s, We) && h.push(s.name);
...
${te.errorWarning(
    ...
    if (e.to ? e.to && typeof e.to == "string" ? a.push(e.to) : e.to && x(e.to, We) ? a.push(e.to.name) : e.to && Array.isArray(e.to) && e.to.forEach((q) => {
      typeof q == "string" ? a.push(q) : x(q, We) && a.push(q.name);
