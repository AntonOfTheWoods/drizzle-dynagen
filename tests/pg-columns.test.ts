...
test("alter column change name #1", async () => {
  const schema1 = {
    users: pgTable("users", {
      id: serial("id").primaryKey(),
      name: text("name"),
  ...
  const { statements } = await diffTestSchemas(schema1, schema2, ["public.users.name->public.users.name1"]);
...
test("alter column change name #2", async () => {
  const schema1 = {
    users: pgTable("users", {
      id: serial("id").primaryKey(),
      name: text("name"),
...
test("alter table add composite pk", async () => {
  const schema1 = {
    table: pgTable("table", {
      id1: integer("id1"),
      id2: integer("id2"),
  ...
  expect(sqlStatements[0]).toBe('ALTER TABLE "table" ADD CONSTRAINT "table_id1_id2_pk" PRIMARY KEY("id1","id2");');
});

test("rename table rename column #1", async () => {
  const schema1 = {
    users: pgTable("users", {
      id: integer("id"),
    }),
...
test("with composite pks #1", async () => {
  const schema1 = {
    users: pgTable(
      "users",
      {
...
test("with composite pks #2", async () => {
  const schema1 = {
    users: pgTable("users", {
      id1: integer("id1"),
      id2: integer("id2"),
...
test("with composite pks #3", async () => {
  const schema1 = {
    users: pgTable(
      "users",
      {
  ...
  const { statements } = await diffTestSchemas(schema1, schema2, ["public.users.id2->public.users.id3"]);
...
test("add multiple constraints #1", async () => {
  const t1 = pgTable("t1", {
...
test("add multiple constraints #2", async () => {
  const t1 = pgTable("t1", {
...
test("add multiple constraints #3", async () => {
  const t1 = pgTable("t1", {
    id1: uuid("id1").primaryKey().defaultRandom(),
    id2: uuid("id2").primaryKey().defaultRandom(),
    id3: uuid("id3").primaryKey().defaultRandom(),
...
test("varchar and text default values escape single quotes", async () => {
  ...
  expect(sqlStatements[0]).toStrictEqual("ALTER TABLE \"table\" ADD COLUMN \"text\" text DEFAULT 'escape''s quotes';");
