...
test("enums #6", async () => {
  ...
  const { statements, sqlStatements } = await diffTestSchemas(from, to, ["folder1.enum->folder2.enum"]);
...
test("enums #11", async () => {
  ...
  const { statements, sqlStatements } = await diffTestSchemas(from, to, ["folder1.enum->public.enum"]);
...
test("enums #12", async () => {
  ...
  const { statements, sqlStatements } = await diffTestSchemas(from, to, ["public.enum->folder1.enum"]);
...
test("enums #13", async () => {
  ...
  const { statements, sqlStatements } = await diffTestSchemas(from, to, ["public.enum1->public.enum2"]);
...
test("enums #14", async () => {
  ...
  const { statements, sqlStatements } = await diffTestSchemas(from, to, ["folder1.enum1->folder2.enum2"]);
...
test("enums #17", async () => {
  ...
  const { statements, sqlStatements } = await diffTestSchemas(from, to, ["public.enum1->schema.enum1"]);
...
test("enums #18", async () => {
  ...
  const { statements, sqlStatements } = await diffTestSchemas(from, to, ["schema1.enum1->schema2.enum2"]);
...
test("enums #19", async () => {
  ...
  expect(sqlStatements.length).toBe(1);
  expect(sqlStatements[0]).toStrictEqual("CREATE TYPE \"public\".\"my_enum\" AS ENUM('escape''s quotes');");
});
...
test("drop enum value", async () => {
  ...
  expect(statements[0]).toStrictEqual({
    columnsWithEnum: [],
    deletedValues: ["value2"],
    name: "enum",
    newValues: ["value1", "value3"],
    schema: "public",
...
test("shuffle enum values", async () => {
  ...
  expect(statements[0]).toStrictEqual({
    ...
    deletedValues: ["value3"],
    name: "enum",
    newValues: ["value1", "value3", "value2"],
    schema: "public",
    type: "alter_type_drop_value",
  });
});
