...
test("create table and view #3", async () => {
  ...
  const to = {
    users: users,
    view1: pgView("some_view1", { id: integer("id") })
      .with({
        checkOption: "local",
...
test("create table and view #6", async () => {
  ...
  const to = {
    users: users,
    view1: pgView("some_view", { id: integer("id") })
...
test("create table and materialized view #3", async () => {
  ...
  const to = {
    ...
    view2: pgMaterializedView("some_view2")
...
\t"id" integer PRIMARY KEY NOT NULL
  ...
  expect(sqlStatements[1]).toBe(`CREATE MATERIALIZED VIEW "public"."some_view1" AS (SELECT * FROM "users");`);
  expect(sqlStatements[2]).toBe(
...
test("create table and materialized view #5", async () => {
  ...
  const to = {
    users: users,
    view1: pgMaterializedView("some_view", { id: integer("id") })
      .with({ autovacuumFreezeMinAge: 14 })
      .as(sql`SELECT * FROM ${users}`),
  };
...
test("add with option to materialized view #1", async () => {
  ...
  const to = {
    users,
    view: pgMaterializedView("some_view")
      .with({ autovacuumMultixactFreezeMaxAge: 3 })
      .as((qb) => qb.select().from(users)),
  };
...
test("drop with option from view #1", async () => {
  ...
  const from = {
    users,
    view: pgView("some_view")
      .with({ checkOption: "cascaded", securityBarrier: true, securityInvoker: true })
      .as((qb) => qb.select().from(users)),
  };
...
test("drop with option from materialized view #1", async () => {
  ...
  const from = {
    users,
    view: pgMaterializedView("some_view")
      .with({ autovacuumEnabled: true, autovacuumFreezeMaxAge: 10 })
      .as((qb) => qb.select().from(users)),
  };
...
test("alter with option in view #1", async () => {
  ...
  const from = {
    users,
    view: pgView("some_view")
      .with({ securityBarrier: true, securityInvoker: true })
      .as((qb) => qb.select().from(users)),
  };
  ...
  expect(sqlStatements.length).toBe(1);
  expect(sqlStatements[0]).toBe(`ALTER VIEW "public"."some_view" RESET (security_invoker);`);
});
...
test("alter with option in materialized view #1", async () => {
  ...
  const from = {
    users,
    view: pgMaterializedView("some_view")
      .with({ autovacuumEnabled: true, autovacuumVacuumScaleFactor: 1 })
      .as((qb) => qb.select().from(users)),
  };
  ...
  expect(sqlStatements.length).toBe(1);
  expect(sqlStatements[0]).toBe(`ALTER MATERIALIZED VIEW "public"."some_view" RESET (autovacuum_vacuum_scale_factor);`);
});
...
test("alter with option in view #2", async () => {
  ...
  const from = {
    users,
    view: pgView("some_view")
      .with({ checkOption: "local", securityBarrier: true, securityInvoker: true })
      .as((qb) => qb.selectDistinct().from(users)),
  };
  ...
  expect(sqlStatements[0]).toBe(`ALTER VIEW "public"."some_view" SET (check_option = cascaded);`);
});
...
test("alter with option in materialized view #2", async () => {
  ...
  const from = {
    users,
    view: pgMaterializedView("some_view")
      .with({ autovacuumEnabled: true, fillfactor: 1 })
      .as((qb) => qb.select().from(users)),
  };
  ...
  const to = {
    users,
    view: pgMaterializedView("some_view")
      .with({ autovacuumEnabled: false, fillfactor: 1 })
      .as((qb) => qb.select().from(users)),
  };
  ...
  expect(sqlStatements.length).toBe(1);
  expect(sqlStatements[0]).toBe(`ALTER MATERIALIZED VIEW "public"."some_view" SET (autovacuum_enabled = false);`);
});
...
test('alter view ".as" value', async () => {
  ...
  const from = {
    users,
    view: pgView("some_view", { id: integer("id") })
      .with({
        checkOption: "local",
        securityBarrier: true,
        securityInvoker: true,
...
test('alter materialized view ".as" value', async () => {
  ...
  const from = {
    users,
    view: pgMaterializedView("some_view", { id: integer("id") })
      .with({
        autovacuumVacuumCostLimit: 1,
      })
      .as(sql`SELECT '123'`),
...
test("alter tablespace - materialize", async () => {
  ...
  expect(sqlStatements.length).toBe(1);
  expect(sqlStatements[0]).toBe(`ALTER MATERIALIZED VIEW "public"."some_view" SET TABLESPACE new_tablespace;`);
});
...
test("drop tablespace - materialize", async () => {
  ...
  expect(sqlStatements.length).toBe(1);
  expect(sqlStatements[0]).toBe(`ALTER MATERIALIZED VIEW "public"."some_view" SET TABLESPACE pg_default;`);
});
...
test("set existing - materialized", async () => {
  ...
  const to = {
    users,
    view: pgMaterializedView("new_some_view", { id: integer("id") })
      .with({
        autovacuumVacuumCostLimit: 1,
        autovacuumFreezeMinAge: 1,
      })
...
test("set existing", async () => {
  ...
  const to = {
    users,
    view: pgView("new_some_view", { id: integer("id") })
      .with({
        checkOption: "cascaded",
        securityBarrier: true,
      })
...
test("alter using - materialize", async () => {
  ...
  expect(sqlStatements.length).toBe(1);
  expect(sqlStatements[0]).toBe(`ALTER MATERIALIZED VIEW "public"."some_view" SET ACCESS METHOD "new_using";`);
});
...
test("drop using - materialize", async () => {
  ...
  expect(sqlStatements.length).toBe(1);
  expect(sqlStatements[0]).toBe(`ALTER MATERIALIZED VIEW "public"."some_view" SET ACCESS METHOD "heap";`);
});
