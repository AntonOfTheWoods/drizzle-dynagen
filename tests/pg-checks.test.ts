...
import { JsonCreateTableStatement } from "../lib/pg/jsonStatements";
import { expect, test } from "vitest";
import { diffTestSchemas } from "./schemaDiffer";

test("create table with check", async () => {
  const to = {
    users: pgTable(
      "users",
      {
...
test("add check contraint to existing table", async () => {
  const from = {
    users: pgTable("users", {
      id: serial("id").primaryKey(),
      age: integer("age"),
  ...
  expect(sqlStatements.length).toBe(1);
  expect(sqlStatements[0]).toBe(`ALTER TABLE "users" ADD CONSTRAINT "some_check_name" CHECK ("users"."age" > 21);`);
});

test("drop check contraint in existing table", async () => {
  const from = {
    users: pgTable(
      "users",
      {
  ...
  expect(sqlStatements.length).toBe(1);
  expect(sqlStatements[0]).toBe(`ALTER TABLE "users" DROP CONSTRAINT "some_check_name";`);
});

test("rename check constraint", async () => {
  const from = {
    users: pgTable(
      "users",
      {
  ...
  expect(sqlStatements[0]).toBe(`ALTER TABLE "users" DROP CONSTRAINT "some_check_name";`);
  expect(sqlStatements[1]).toBe(`ALTER TABLE "users" ADD CONSTRAINT "new_check_name" CHECK ("users"."age" > 21);`);
});
...
test("alter check constraint", async () => {
  ...
  expect(sqlStatements.length).toBe(2);
  expect(sqlStatements[0]).toBe(`ALTER TABLE "users" DROP CONSTRAINT "some_check_name";`);
  expect(sqlStatements[1]).toBe(`ALTER TABLE "users" ADD CONSTRAINT "new_check_name" CHECK ("users"."age" > 10);`);
});

test("alter multiple check constraints", async () => {
  const from = {
    users: pgTable(
      "users",
      {
  ...
  expect(sqlStatements[0]).toBe(`ALTER TABLE "users" DROP CONSTRAINT "some_check_name_1";`);
  expect(sqlStatements[1]).toBe(`ALTER TABLE "users" DROP CONSTRAINT "some_check_name_2";`);
  expect(sqlStatements[2]).toBe(`ALTER TABLE "users" ADD CONSTRAINT "some_check_name_3" CHECK ("users"."age" > 21);`);
  expect(sqlStatements[3]).toBe(
    `ALTER TABLE "users" ADD CONSTRAINT "some_check_name_4" CHECK ("users"."name" != 'Alex');`,
  );
});

test("create checks with same names", async () => {
  const to = {
    users: pgTable(
      "users",
      {
