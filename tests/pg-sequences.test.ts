...
test("rename sequence", async () => {
  ...
  const { statements, sqlStatements } = await diffTestSchemas(from, to, ["public.name->public.name_new"]);
  ...
  expect(sqlStatements).toStrictEqual(['ALTER SEQUENCE "public"."name" RENAME TO "name_new";']);
});
...
test("rename sequence in custom schema", async () => {
  ...
  const { statements, sqlStatements } = await diffTestSchemas(from, to, ["custom.name->custom.name_new"]);
  ...
  expect(sqlStatements).toStrictEqual(['ALTER SEQUENCE "custom"."name" RENAME TO "name_new";']);
});
...
test("move sequence between schemas #1", async () => {
  ...
  const { statements, sqlStatements } = await diffTestSchemas(from, to, ["public.name->custom.name"]);
  ...
  expect(sqlStatements).toStrictEqual(['ALTER SEQUENCE "public"."name" SET SCHEMA "custom";']);
});
...
test("move sequence between schemas #2", async () => {
  ...
  const { statements, sqlStatements } = await diffTestSchemas(from, to, ["custom.name->public.name"]);
  ...
  expect(sqlStatements).toStrictEqual(['ALTER SEQUENCE "custom"."name" SET SCHEMA "public";']);
});
