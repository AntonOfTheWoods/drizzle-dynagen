...
export function diffForRenamedTables(pairs: any) {
  // raname table1 to name of table2, so we can apply diffs
  const renamed = pairs.map((it: any) => {
    const from = it.from;
    const to = it.to;
  ...
  const altered = renamed.map((pair: any) => {
...
function diffForRenamedTable(t1: any, t2: any) {
  t1.name = t2.name;
...
export function diffForRenamedColumn(t1: any, t2: any) {
...
const mapArraysDiff = (source: any, diff: any) => {
  const sequence: any = [];
  let sourceIndex = 0;
...
export function diffSchemasOrTables(left: any, right: any) {
  ...
  const added = result.filter((it: any) => it[0].endsWith("__added")).map((it: any) => it[1]);
  const deleted = result.filter((it: any) => it[0].endsWith("__deleted")).map((it: any) => it[1]);

  return { added, deleted };
}

export function diffIndPolicies(left: any, right: any) {
  left = JSON.parse(JSON.stringify(left));
  right = JSON.parse(JSON.stringify(right));
...
export function diffColumns(left: any, right: any) {
  left = JSON.parse(JSON.stringify(left));
  right = JSON.parse(JSON.stringify(right));
  const result: any = diff(left, right) ?? {};
  ...
  const alteredTables = Object.fromEntries(
    Object.entries(result)
      ...
      .map((tableEntry: any) => {
        ...
        return [tableEntry[0], { name: table.name, schema: table.schema, ...tableEntry[1] }];
      }),
  );
...
export function applyJsonDiff(json1: any, json2: any) {
  ...
  for (const [tableKey, tableValue] of Object.entries(difference.tables)) {
    const table = difference.tables[tableKey];
    const columns = (tableValue as any).columns || {};
    ...
    if ("name" in table && "schema" in table && Object.keys(table).length === 2) {
  ...
  const alteredEnums = enumsEntries
    ...
    .map((it: any) => {
      ...
      const deletedValues = sequence.filter((it: any) => it.type === "removed").map((it: any) => it.value);
  ...
  const alteredSequences = sequencesEntries
    .filter((it: any) => !(it[0].includes("__added") || it[0].includes("__deleted")) && "values" in it[1])
    .map((it) => {
  ...
  const alteredViews = viewsEntries
    .filter((it: any) => !(it[0].includes("__added") || it[0].includes("__deleted")))
    .map(([nameWithSchema, view]: [any, any]) => {
      const deletedWithOption = view.with__deleted;

      const addedWithOption = view.with__added;

      const deletedWith = Object.fromEntries(
        Object.entries(view.with || {})
      ...
      const alterWith = Object.fromEntries(
        Object.entries(view.with || {})
          .filter((it: any) => typeof it[1].__old !== "undefined" && typeof it[1].__new !== "undefined")
          .map((it: any) => {
  ...
  const alteredTablesWithColumns = Object.values(difference.tables).map((table) => {
...
const findAlternationsInTable = (table: any) => {
  ...
  const deletedIndexes = Object.fromEntries(
    Object.entries(table.indexes__deleted || {})
      .concat(Object.entries(table.indexes || {}).filter((it) => it[0].includes("__deleted")))
      .map((entry) => [entry[0].replace("__deleted", ""), entry[1]]),
  );

  const addedIndexes = Object.fromEntries(
    Object.entries(table.indexes__added || {})
      .concat(Object.entries(table.indexes || {}).filter((it) => it[0].includes("__added")))
  ...
  const deletedPolicies = Object.fromEntries(
    Object.entries(table.policies__deleted || {})
      .concat(Object.entries(table.policies || {}).filter((it) => it[0].includes("__deleted")))
      .map((entry) => [entry[0].replace("__deleted", ""), entry[1]]),
  );

  const addedPolicies = Object.fromEntries(
    Object.entries(table.policies__added || {})
      .concat(Object.entries(table.policies || {}).filter((it) => it[0].includes("__added")))
      .map((entry) => [entry[0].replace("__added", ""), entry[1]]),
  );
  ...
  const deletedForeignKeys = Object.fromEntries(
    Object.entries(table.foreignKeys__deleted || {})
      .concat(Object.entries(table.foreignKeys || {}).filter((it) => it[0].includes("__deleted")))
      .map((entry) => [entry[0].replace("__deleted", ""), entry[1]]),
  );

  const addedForeignKeys = Object.fromEntries(
    Object.entries(table.foreignKeys__added || {})
      .concat(Object.entries(table.foreignKeys || {}).filter((it) => it[0].includes("__added")))
      .map((entry) => [entry[0].replace("__added", ""), entry[1]]),
  );

  const alteredForeignKeys = Object.fromEntries(
    Object.entries(table.foreignKeys || {})
      .filter((it) => !it[0].endsWith("__added") && !it[0].endsWith("__deleted"))
...
const alternationsInColumn = (column: any) => {
  ...
  const result = altered
    ...
    .map((it) => {
      ...
      if ("default__added" in it) {
        const { default__added: _, ...others } = it;
        return {
          ...others,
      ...
      if ("default__deleted" in it) {
        const { default__deleted: _, ...others } = it;
        return {
          ...others,
    ...
    .map((it) => {
      ...
      if ("generated__added" in it) {
        const { generated__added: _, ...others } = it;
        return {
          ...others,
      ...
      if ("generated__deleted" in it) {
        const { generated__deleted: _, ...others } = it;
        return {
          ...others,
    ...
    .map((it) => {
      ...
      if ("identity__added" in it) {
        const { identity__added: _, ...others } = it;
        return {
          ...others,
      ...
      if ("identity__deleted" in it) {
        const { identity__deleted: _, ...others } = it;
        return {
          ...others,
    ...
    .map((it) => {
      ...
      if ("notNull__added" in it) {
        const { notNull__added: _, ...others } = it;
        return {
          ...others,
      ...
      if ("notNull__deleted" in it) {
        const { notNull__deleted: _, ...others } = it;
        return {
          ...others,
    ...
    .map((it) => {
      ...
      if ("typeSchema__added" in it) {
        const { typeSchema__added: _, ...others } = it;
        return {
          ...others,
      ...
      if ("typeSchema__deleted" in it) {
        const { typeSchema__deleted: _, ...others } = it;
        return {
          ...others,
    ...
    .map((it) => {
      ...
      if ("onUpdate__added" in it) {
        const { onUpdate__added: _, ...others } = it;
        return {
          ...others,
      ...
      if ("onUpdate__deleted" in it) {
        const { onUpdate__deleted: _, ...others } = it;
        return {
          ...others,
    ...
    .map((it) => {
      ...
      if ("autoincrement__added" in it) {
        const { autoincrement__added: _, ...others } = it;
        return {
          ...others,
      ...
      if ("autoincrement__deleted" in it) {
        const { autoincrement__deleted: _, ...others } = it;
        return {
          ...others,
