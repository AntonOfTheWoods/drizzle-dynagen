...
const column = object({
  ...
  identity: sequenceSchema.merge(object({ type: enumType(["always", "byDefault"]) })).optional(),
}).strict();
...
export const backwardCompatiblePgSchema = union([pgSchemaV5, pgSchemaV6, pgSchema]);

export const PgSquasher = {
  squashIdx: (idx: Index) => {
    index.parse(idx);
    return `${idx.name};${idx.columns
      .map((c) => `${c.expression}--${c.isExpression}--${c.asc}--${c.nulls}--${c.opclass ? c.opclass : ""}`)
  ...
  squashPolicy: (policy: Policy) => {
    return `${policy.name}--${policy.as}--${policy.for}--${policy.to?.join(",")}--${policy.using}--${
      policy.withCheck
    }--${policy.on}`;
  },
  ...
  unsquashFK: (input: string): ForeignKey => {
    const [name, tableFrom, columnsFromStr, tableTo, columnsToStr, onUpdate, onDelete, schemaTo] = input.split(";");
  ...
  squashIdentity: (seq: Omit<Sequence, "schema"> & { type: "always" | "byDefault" }) => {
    return `${seq.name};${seq.type};${seq.minValue};${seq.maxValue};${seq.increment};${seq.startWith};${seq.cache};${
      seq.cycle ?? ""
    }`;
  },
  unsquashIdentity: (seq: string): Omit<Sequence, "schema"> & { type: "always" | "byDefault" } => {
    const splitted = seq.split(";");
    return {
      name: splitted[0],
...
export const squashPgScheme = (json: PgSchema, action?: "push" | undefined): PgSchemaSquashed => {
  const mappedTables = Object.fromEntries(
    Object.entries(json.tables).map((it) => {
      const squashedIndexes = mapValues(it[1].indexes, (index) => {
        return action === "push" ? PgSquasher.squashIdxPush(index) : PgSquasher.squashIdx(index);
      });
      ...
      const mappedColumns = Object.fromEntries(
        Object.entries(it[1].columns).map((it) => {
          const mappedIdentity = it[1].identity ? PgSquasher.squashIdentity(it[1].identity) : undefined;
          return [
            it[0],
            {
              ...it[1],
      ...
      const squashedPolicies = mapValues(it[1].policies, (policy) => {
        return action === "push" ? PgSquasher.squashPolicyPush(policy) : PgSquasher.squashPolicy(policy);
      });
      const squashedChecksContraints = mapValues(it[1].checkConstraints, (check) => {
