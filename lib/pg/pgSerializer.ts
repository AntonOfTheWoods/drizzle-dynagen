...
import { camelCase, IntrospectStage, IntrospectStatus, withStyle } from "./lib";
...
export const generatePgSnapshot = (
  ...
  for (const table of tables) {
    ...
    columns.forEach((column) => {
      ...
      if (column.isUnique) {
        ...
        if (typeof existingUnique !== "undefined") {
          console.error(
            `\n${withStyle.errorWarning(`We've found duplicated unique constraint names in ${tableName} table.
    ...
    indexes.forEach((value) => {
          ...
          console.log(
            `\n${withStyle.errorWarning(
              `You are specifying an index on the ${name} column inside the ${
                ...
                .join(
                  ", ",
                )}].\n\nYou can specify it using current syntax: ${`index("${value.config.name}").using("${value.config.method}", table.${name}.op("${
                vectorOps[0]
      ...
      const indexColumns: IndexColumnType[] = columns.map((it): IndexColumnType => {
        if (is(it, SQL)) {
          return {
    ...
    checks.forEach((check) => {
      ...
      if (typeof checksInTable[`"${schema ?? "public"}"."${tableName}"`] !== "undefined") {
        if (checksInTable[`"${schema ?? "public"}"."${tableName}"`].includes(check.name)) {
          console.log(
            `\n${withStyle.errorWarning(
              `We've found duplicated check constraint name across ${schema ?? "public"} schema in ${
                tableName
  ...
  for (const policy of policies) {
    // @ts-expect-error - no idea why this isn't specified in the interface...
      ...
      if (is(selectedFields[key], PgColumn)) {
        ...
        if (column.isUnique) {
          ...
          if (typeof existingUnique !== "undefined") {
            ...
            The unique constraint ${column.uniqueName} on the ${
              column.name
            } column is confilcting with a unique constraint name already defined for ${existingUnique.columns.join(
              ",",
            )} columns\n`,
              )}`,
            );
    ...
    .map((row) => {
      ...
      WHERE tc.table_name = '${tableName}' and constraint_schema = '${tableSchema}';`,
          ...
          for (const columnResponse of tableResponse) {
            ...
            const columnType: string = columnResponse.data_type;
      ...
      return new Promise(async (res, rej) => {
        ...
        try {
          ...
          for (const viewResponse of viewResponses) {
            ...
            const columnType: string = viewResponse.data_type;
...
WHERE
    ...
    AND c.relname = '${viewName}';`);
          ...
          const withOption = Object.values(resultWith).length
            ? Object.fromEntries(Object.entries(resultWith).map(([key, value]) => [camelCase(key), value]))
            : undefined;
