...
import { extendedTypeOf, roundObj } from "./util";
...
class JsonDiff {
  ...
  objectDiff(obj1: any, obj2: any) {
    ...
    for (const [key, value] of Object.entries(obj1) as any[]) {
  ...
  findMatchingObject(item: any, index: any, fuzzyOriginals: any) {
    ...
    for (const [key, { item: candidate, index: matchIndex }] of Object.entries(fuzzyOriginals) as any[]) {
      if (key !== "__next") {
  ...
  scalarize(array: any, originals: any, fuzzyOriginals?: any) {
    ...
    if (fuzzyOriginals) {
      ...
      for (const [key, match] of Object.entries(keyScores) as any[]) {
        ...
        case "equal":
          for (i = i1, end = i2, asc = i1 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
            ...
            if (this.isScalarized(item, originals1)) {
              ...
              const change: any = this.diff(item1, item2);
              if (!change.equal) {
...
// export function diffString(obj1: any, obj2: any, options = {}) {
